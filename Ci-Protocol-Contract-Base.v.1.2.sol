...
contract ERC20 is IERC20, Master {
    ...
    function transfer(address to, uint256 value) public returns (bool) {
        ...
        else{ agent = AccountsAll[to].AgentWallet; } //Keep agent
...
contract CiProtocol is ERC20, CiStatistics{
        ...
        while( remainToEmit > 0 ){
            if(getLowerForSaleDate() != 0){
                ...
                if( SellerForSale == remainToEmit ){ 
                    ...
                    PayToMaster(Buyer, emitedToBuyer, CoinPrice);
                    remainToEmit = 0;
                }
                ...
                PayToMaster(Buyer, remainToEmit, CoinPrice);
                remainToEmit = 0;
            }
        }
    ...
    event PayedToMaster_event(address indexed Buyer, address indexed MasterWallet, uint CoinPayed, uint CoinPrice, uint Amount);
    
    function PayToMaster(address Buyer, uint CoinPayed, uint CoinPrice) internal {
        uint WeiToPay = Coin2ETH_internal(CoinPayed, CoinPrice);
        ...
        emit PayedToMaster_event(Buyer, master, CoinPayed, CoinPrice, amountToMaster);
    ...
    //Direct coin sale
    ////////////////////////////////////////////
    event CoinSale_event( address indexed Seller, uint CoinAmount, uint CoinPrice); 
    ...
    function CoinSale(uint256 CoinAmount, uint timestamp) external{
        ...
        if(Deposit > 0){
            if( Deposit >= WeiToPay){
                PayToSeller(msg.sender, CoinAmount, CoinPrice);
                burn( msg.sender, CoinAmount);
                emit CoinSale_event(msg.sender, CoinAmount, CoinPrice);
            }
            else{
                uint DepositToCoin = ETH2Coin_internal(Deposit, CoinPrice);   
                PayToSeller(msg.sender, DepositToCoin, CoinPrice);
