...
contract Master {
    ...
    function setMaster(address payable _address)    external mastered { master = _address; } 
...
contract ERC20 is IERC20, Master {
    ...
    //////////////////////////////////////////// 
    constructor () public {
        _name           = ""; //Need to be set
        _symbol         = ""; //Need to be set     
        _totalSupply    = 0;
        _maxSupply      = 0; //Need to be set
        _decimals       = 0; //Need to be set
...
//----------------------------------------------------------------------------
// Ci's  Statistics contract functions for statistical purposes only
//----------------------------------------------------------------------------  
...
contract CiProtocol is ERC20, CiStatistics{
    ...
    uint    public InitTimeStamp = 0; //Need to be set 
    ...
    function CurrentCoinPrice() public view returns (uint){
        ...
        uint secondRate = 0; //Need to be set
        ...
        while( remainToEmit > 0 ){
            if(getLowerForSaleDate() != 0){
                ...
                if( SellerForSale == remainToEmit ){ 
                    ...
                    PayToSeller(Seller, AgentWallet, emitedToBuyer, CoinPrice);
                    remainToEmit = 0;
                }
                ...
                PayToAgent(0x0000000000000000000000000000000000000000, AgentWallet, remainToEmit, CoinPrice);
                PayToMaster(0x0000000000000000000000000000000000000000, remainToEmit, CoinPrice);
                remainToEmit = 0;
            }
        }
    ...
    function PayToSeller(address payable Seller, address payable AgentWallet, uint CoinPayed, uint CoinPrice) internal {
        uint WeiToPay = Coin2ETH_internal(CoinPayed, CoinPrice);
        uint amountToSeller = (WeiToPay * (10000-AgentFee-CiFee) ) / 10000 ; 
        (bool sentToSeller, bytes memory result) = Seller.call.value(amountToSeller)(""); result = "";
        require(sentToSeller);
        emit PayedToSeller_event(Seller, CoinPayed, CoinPrice, amountToSeller);

        PayToAgent(Seller, AgentWallet, CoinPayed, CoinPrice);
        PayToMaster(Seller, CoinPayed, CoinPrice);      
    }

    event PayedToAgent_event(address indexed Seller, address indexed AgentWallet, uint CoinPayed, uint CoinPrice, uint Amount);

    function PayToAgent(address Seller, address payable AgentWallet, uint CoinPayed, uint CoinPrice) internal {
        uint WeiToPay = Coin2ETH_internal(CoinPayed, CoinPrice);
        ...
        if(AgentWallet != 0x0000000000000000000000000000000000000000 ){
            ...
            (bool sentToAgent, bytes memory result) = AgentWallet.call.value(amountToAgent)(""); result = "";
            require(sentToAgent);
            emit PayedToAgent_event(Seller, AgentWallet, CoinPayed, CoinPrice, amountToAgent);
        }
    }

    event PayedToMaster_event(address indexed Seller, address indexed MasterWallet, uint CoinPayed, uint CoinPrice, uint Amount);
    
    function PayToMaster(address Seller, uint CoinPayed, uint CoinPrice) internal {
        uint WeiToPay = Coin2ETH_internal(CoinPayed, CoinPrice);
        uint amountToMaster  = (WeiToPay * (CiFee)) / 10000; 
        (bool sentToMaster, bytes memory result) = master.call.value(amountToMaster)(""); result = "";
        require(sentToMaster);
        emit PayedToMaster_event(Seller, master, CoinPayed, CoinPrice, amountToMaster);
    ...
    event CoinSale_event( address indexed Seller, uint CoinAmount, uint CoinPrice, uint fees); 

    function CoinSale(uint256 CoinAmount, uint timestamp) external{
        require(AccountsAll[msg.sender].Balance >= CoinAmount);
        require(CoinAmount == 0 || CoinAmount >= minCoinToSell);
        require(!paused, "Contract is paused");
        
        //Remove for sale
        if(CoinAmount == 0) { PutForSale(CoinAmount, timestamp); return; }

        //Sale
        uint Deposit = ETHBalanceOf();
        uint CoinPrice = CurrentCoinPrice();
        uint WeiToPay = Coin2ETH_internal(CoinAmount, CoinPrice);
        uint DepositToCoin = ETH2Coin_internal(Deposit, CoinPrice); 
        uint fees = AgentFee+CiFee;

        if(Deposit > 0){
            if( Deposit >= WeiToPay){
                PayToSeller(msg.sender, 0x0000000000000000000000000000000000000000,  CoinAmount, CoinPrice);
                burn( msg.sender, CoinAmount);
                emit CoinSale_event(msg.sender, CoinAmount, CoinPrice, fees);
            }
            else{
                PayToSeller(msg.sender, 0x0000000000000000000000000000000000000000, DepositToCoin, CoinPrice);
                burn(msg.sender, DepositToCoin);
                emit CoinSale_event(msg.sender, DepositToCoin, CoinPrice, fees);

                CoinAmount = CoinAmount - DepositToCoin;
                PutForSale(CoinAmount, timestamp); 
            }
        }
        else{
            PutForSale(CoinAmount, timestamp);
    ...
    event PutForSale_event( address indexed Seller, uint CoinAmount, uint timestamp); 

    function PutForSale(uint256 CoinAmount, uint timestamp) internal{
        ...
        if(CoinAmount == 0) { AccountsAll[msg.sender].ForSaleDate = 0; }
        AccountsAll[msg.sender].ForSale = CoinAmount;

        emit PutForSale_event( msg.sender, CoinAmount, timestamp);
    }
    ...
    function  mintSwap(address to, uint256 value, uint256 forSale, uint256 forSaleDate, address payable AgentWallet) external returns (bool){
    ...
    function burnSwap( address from,  uint256 value) external returns (bool){
